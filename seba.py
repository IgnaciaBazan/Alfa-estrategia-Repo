...
class StrategicGoal(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    dimension = Column(String(64), nullable=False)          # 'LIDERAZGO', etc.
    objetivo = Column(String(512), nullable=False)          # texto del objetivo estratégico
...
# GET (listar por objetivo+dimensión)
...
# POST (crear) – solo editor
@app.post("/strategic-goals", response_model=StrategicGoalOut)
def create_strategic_goal(payload: StrategicGoalIn,
                          db: Session = Depends(get_db),
                          user = Depends(require_role("editor"))):   # usa tu guard de rol
...
# PUT (actualizar) – solo editor
@app.put("/strategic-goals/{sid}", response_model=StrategicGoalOut)
def update_strategic_goal(sid: int, payload: StrategicGoalUpdate,
                          db: Session = Depends(get_db),
...
def delete_strategic_goal(sid: int,
                          db: Session = Depends(get_db),
                          user = Depends(require_role("editor"))):
    rec = db.query(StrategicGoal).get(sid)
