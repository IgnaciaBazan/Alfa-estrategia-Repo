...
function navigateHash(newHash) {
  if (location.hash === newHash) {
    router(); // si ya está igual, forzamos
    return;
  }
  location.hash = newHash;
  // fallback: en algunos navegadores/estados el hashchange se pierde
  setTimeout(() => {
...
// guardar/usar el contexto de metas e ir a indicadores
...
function loadScriptOnce(src) {
  return new Promise((resolve, reject) => {
    ...
    s.src = src;
    s.type = 'module'; // OK si tu plan_form.js usa módulos; si no, puedes quitar esta línea.
    s.dataset.dyn = src;
    s.onload = resolve;
...
// ===== Helpers API para Objectives / Goals / Indicators / Evidences =====
...
async function ensureObjectiveByName(name, accionesOptional) {
  ...
  // Si no existe, crea uno 4 años desde año actual (plan cuatrienal)
  const now = new Date();
...
async function apiListGoalsByObjective(objectiveId) {
  ...
  return res.json(); // [{id,title,description,year,objective_id}]
}
...
async function apiListIndicatorsByGoal(goalId) {
  ...
  return res.json(); // [{id,title,target,unit,goal_id}]
}

async function apiCreateIndicator(goalId, payload) {
  // limpia payload sin null/undefined
  const clean = {};
  ...
  if (payload.target !== '' && payload.target !== null && payload.target !== undefined) {
    clean.target = String(payload.target).trim();  // ← siempre string
  }
...
// (placeholder de filtros si luego decides implementarlos)
...
async function showPlanList(dimensionValue) {
  ...
  $list.addEventListener('click', (ev) => {
    ...
    if (btn.dataset.act === 'evi')  showEvidenceUpload(dimensionValue, objetivo); // ← NUEVO
  });
...
// ====== IndexedDB para Evidencias (local) ======
...
// ====== Vista Evidencias (local-only, opcional carpeta del PC) ======
...
function paintTable(indics) {
  ...
  // restaurar valores previos (opcional, persistencia local)
  indics.forEach(x => {
...
document.getElementById('btnAddIndic')?.addEventListener('click', async () => {
  ...
  const unidad = document.getElementById('indUnidad').value.trim();    // puede quedar vacío
  const targetStr = document.getElementById('indTarget').value.trim(); // ← string directo

  if (!nombre) { alert('Ingresa el nombre del indicador.'); return; }

  // Validación sencilla por unidad (opcional)
  ...
  // acciones de la tabla
    ...
    // escuchar entradas en los paneles
...
async function showStrategicGoalsEditor(dimensionValue, objetivo) {
  ...
  // Arma lista de años para el selector (si hay start/end en el objetivo, úsalo)
  ...
  async function refreshTable() {
    const metas = await apiListGoalsByObjective(objectiveId); // goals del objetivo
    if (!metas.length) {
  ...
  // Guardar meta (solo Goal; sin asociar a acción)
  ...
  $tb.addEventListener('click', async (e) => {
    ...
    // 👇 NUEVO: navegar a Indicadores desde Metas
    if (btn.dataset.act === 'toIndicators') {
      const goalId = btn.dataset.goal;
      // guarda contexto para el botón Volver en Indicadores
      window.__metasCtx = { dimension: dimensionValue, objetivo };
      sessionStorage.setItem('metasCtx', JSON.stringify(window.__metasCtx));
      navigateHash(`#/indicadores/goal/${goalId}`);  // fuerza router si el hash no cambia
      return;
    }
...
async function showIndicadoresPage(id) {
  ...
  document.getElementById('btnUp')?.addEventListener('click', async () => {
    ...
    const res = await fetch(`${API}/indicators/${id}/evidences`, {
      method: 'POST',
      headers: { ...authHeaders() }, // no pongas Content-Type (deja que lo setee el browser)
      body: fd
    });
